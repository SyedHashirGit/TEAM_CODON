...
window.deleteVolunteer = function(id, name) {
  ...
  _deleteAction = async()=>{
    ...
    await logAction("VOLUNTEER_DELETED_BY_ADMIN", { volunteerId: id, name });
    await pushNotif("admin", "Admin", "admin_action", `Admin deleted volunteer ${name}`);
...
window.openRejectVolunteer = function(id, name, email) {
  document.getElementById("reject-vol-id").value = id;
  document.getElementById("reject-vol-name").value = name;
  document.getElementById("reject-vol-email").value = email;
  document.getElementById("reject-reason").value = "";
  document.getElementById("reject-modal").style.display = "flex";
};

window.confirmRejectAction = async function() {
  const id = document.getElementById("reject-vol-id").value;
  const name = document.getElementById("reject-vol-name").value;
  const email = document.getElementById("reject-vol-email").value;
  const reason = document.getElementById("reject-reason").value.trim();
  
  if (!reason) {
    toast("Reason required", "Please provide a reason to email the volunteer.", "warning");
    return;
  }
  ...
  try {
    await rejectWaitlistVolunteer(id, name, email, reason);
    toast("Rejected", `${name} has been rejected and notified.`, "success");
...
window.saveParticipant = async function(e) {
  ...
  if (id) {
    ...
    await logAction("PARTICIPANT_EDITED_BY_ADMIN", { participantId: id, name: data.name });
    await pushNotif("admin", "Admin", "admin_edit", `Admin edited participant ${data.name}`);
    ...
    await logAction("PARTICIPANT_CREATED_BY_ADMIN", { participantId: newId, name: data.name });
    await pushNotif("admin", "Admin", "admin_action", `Admin created participant ${data.name}`);
...
window.deleteParticipant = function(id, name) {
  ...
  _deleteAction = async()=>{
    ...
    await logAction("PARTICIPANT_DELETED_BY_ADMIN", { participantId: id, name });
    await pushNotif("admin", "Admin", "admin_action", `Admin deleted participant ${name}`);
    toast("Deleted",`${name} removed`,"success");
    loadParticipants();
  };
  document.getElementById("confirm-modal").style.display = "flex";
